...
RUN python3 -c "import numpy as np;print(np.__version__);"
    # # Install open3d to python
# RUN cd ${O3D_DIR} && make pip-package && \
#     python3 -m pip install lib/python_package/pip_package/open3d-0.18.0+0f06a149c-cp310-cp310-manylinux_2_35_x86_64.whl

# # Install CSF to Python
# ENV CSF_DIR=/root/sdp_tph/submodules/CSF 
# RUN cd ${CSF_DIR} && python3 setup.py build && python3 setup.py install  
