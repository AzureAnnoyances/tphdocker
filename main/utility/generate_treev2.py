    ...
    def process_each_coord(self, pcd, grd_pcd, non_grd, coords, w_lin_pcd, h_lin_pcd):
        ...
        for index, coord in enumerate(coord_loop):
            ...
            if n_detected <= 0:
                ...
                trunk_img, crown_img = split_pcd_by2_with_height(
                    multi_tree, 
                ...
                cv2.imwrite(f"{self.sideViewOut}/yolo_{index}.png", h_im_list[0])
                cv2.imwrite(f"{self.sideViewOut}/trunk_{index}.png", trunk_img*255)
                cv2.imwrite(f"{self.sideViewOut}/crown_{index}.png", crown_img*255)
